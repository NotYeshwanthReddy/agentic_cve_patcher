...
def get_checkpointer():
    ...
    try:
        # Try importing from langgraph-checkpoint-sqlite package
        ...
        # Fallback to langgraph.checkpoint.sqlite if available
        try:
        ...
        # Fallback to MemorySaver if SQLite initialization fails
        logger.warning(f"Failed to initialize SQLite checkpointer: {e}, using MemorySaver")
        from langgraph.checkpoint.memory import MemorySaver
        _checkpointer = MemorySaver()
        return _checkpointer
